...
router.post('/fetch-with-twitter-api', async (req, res) => {
  try {
    ...
    // Calculate stats
    const successCount = postsWithTwitterData.filter(p => !p.twitterApiError).length;
    const errorCount = postsWithTwitterData.filter(p => p.twitterApiError).length;

    res.json({
      success: true,
      posts: postsWithTwitterData,
      message: `Fetched ${initialPosts.length} posts from Google Sheets. Twitter API: ${successCount} successful, ${errorCount} failed.`,
      stats: {
        total: postsWithTwitterData.length,
        twitterApiSuccess: successCount,
        twitterApiErrors: errorCount,
      },
    });
  } catch (error) {
    console.error('Fetch with Twitter API error:', error);
    res.status(500).json({
      message: 'Failed to fetch posts with Twitter API',
      error: error.message,
    });
  }
});
