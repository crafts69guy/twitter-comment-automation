...
router.post('/generate', async (req, res) => {
  try {
    const { postId, provider, apiKey } = req.body;

    if (!postId || !provider || !apiKey) {
      return res.status(400).json({
        message: 'Post ID, provider, and API key are required'
      });
    }
...
Generate only the reply text, no quotes or extra formatting:`;
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: 'You are a helpful social media assistant that generates thoughtful, engaging Twitter replies. Be professional, authentic, and add value to conversations.'
      },
      {
        role: 'user',
        content: prompt
...
router.post('/generate-demo', async (req, res) => {
  try {
    ...
    else if (content.includes('azure') || content.includes('cloud')) selectedCategory = 'cloud';
    else if (content.includes('space') || content.includes('mission')) selectedCategory = 'space';
    else if (content.includes('privacy') || content.includes('security')) selectedCategory = 'privacy';

    const categoryComments = demoComments[selectedCategory];
    const generatedComment = categoryComments[Math.floor(Math.random() * categoryComments.length)];
